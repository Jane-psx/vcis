...
const VCISNav = (() => {
  ...
  function getBadges() {
    const badges = {};
    try {
      // Datacheck: count open (unfixed) issues from vcis_issues
      const issues = JSON.parse(localStorage.getItem('vcis_issues') || '[]');
      const openIssues = issues.filter(i => !i.fixed).length;
      if (openIssues > 0) badges.datacheck = { count: openIssues, type: 'amber' };

      // Addendum: count critical/expiring vendors as a proxy for needing review
      const vendors = JSON.parse(localStorage.getItem('vcis_vendors') || '[]');
      const needReview = vendors.filter(v => v.score >= 9).length;
      if (needReview > 0) badges.addendum = { count: needReview, type: 'red' };
    } catch (e) {
